...
#if defined(NDEBUG)
# error "Medcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xdf9c512fd42353ba343df2b6863eabed6b68defd58b39f674dbe9538785fdd3c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Medcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Medcoin Signed Message:\n";
...
{
    // Medcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 250000); // Medcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Medcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Medcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Medcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Medcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x350a757a9a48ee787cf031e64969cb2103547c9db40108cf01d7f13c187afc13");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1524544070;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1888984;

        if (fTestNet)
        {
            block.nTime    = 1524544082;
            block.nNonce   = 4221633;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xf76d037345092f592715aadac8e131807a2fbfe54da9f01eefed52aed4877b2c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc1, 0xb2, 0xdc }; // Medcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MedcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MedcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MedcoinMiner(CWallet *pwallet)
{
    printf("MedcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("medcoin-miner");
    ...
    try { loop {
        ...
        printf("Running MedcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MedcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MedcoinMiner, pwallet));
}
